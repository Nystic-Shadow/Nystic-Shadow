...
def extract_perfect_avatar_dots():
    # Load user screenshot
    screenshot_path = r'C:\Users\SAZUZ\Pictures\Screenshots\Screenshot 2026-08-06 131829.png'
    img = Image.open(screenshot_path).convert('L')
    
    w, h = img.size
    print(f"Screenshot size: {w}x{h}")
    
    # Try cropping left portion (where portrait/avatar resides)
    # If avatar is inside (x: 0..400, y: 0..400)
    avatar_crop = img.crop((0, 0, min(w, 500), min(h, 500)))
    
    # Enhance contrast
    enhancer = ImageEnhance.Contrast(avatar_crop)
    avatar_crop = enhancer.enhance(2.0)
    
    # Target grid resolution: 80x80 grid for crisp non-mushed dots
    grid_w, grid_h = 75, 75
    avatar_small = avatar_crop.resize((grid_w, grid_h), Image.Resampling.LANCZOS)
    
    dots = []
    # Grid spacing in SVG: 3.5px per dot cell -> total size ~ 260px x 260px
    start_x = 45
    start_y = 75
    cell_spacing = 3.6
    ...
    for gy in range(grid_h):
        for gx in range(grid_w):
            val = avatar_small.getpixel((gx, gy))
            # Contrast threshold: only sample distinct subject/feature pixels (val < 160)
            if val < 170:
                # Dot size proportional to darkness or uniform crisp dot
                px = start_x + (gx * cell_spacing) + random.uniform(-0.3, 0.3)
                py = start_y + (gy * cell_spacing) + random.uniform(-0.3, 0.3)
                dots.append((round(px, 1), round(py, 1)))

    print(f"Extracted {len(dots)} crisp dithered avatar dots!")
    return dots

def build_perfect_svgs(is_dark=True):
    dots = extract_perfect_avatar_dots()
    ...
    # Partition dots into 25 clean SMIL animation groups
    random.seed(77)
    random.shuffle(dots)
    
    num_groups = 25
    dots_per_grp = max(1, len(dots) // num_groups)
    groups_xml = []
    
    for i in range(num_groups):
        grp_dots = dots[i*dots_per_grp : (i+1)*dots_per_grp]
        dur = round(random.uniform(2.5, 4.2), 2)
        delay = round(random.uniform(0, 1.8), 2)
        color = accent_cyan if i % 3 == 0 else (accent_purple if i % 3 == 1 else accent_pink)
        
        path_d = " ".join([f"M{d[0]} {d[1]}h2.2v2.2h-2.2z" for d in grp_dots])
        grp_str = f'''    <g fill="{color}">
      <animate attributeName="opacity" values="0.35;1;0.6;1;0.35" dur="{dur}s" begin="{delay}s" repeatCount="indefinite"/>
      <path d="{path_d}"/>
    </g>'''
  ...
  <!-- LEFT COLUMN: Main Hero Title & Avatar Dither Contour -->
  <g transform="translate(10, 10)">
    <!-- Crisp Dithered Avatar Dot Matrix -->
{portrait_dots_html}
...
with open(r'C:\Users\SAZUZ\Nystic-Shadow\dark.svg', 'w', encoding='utf-8') as f:
    f.write(build_perfect_svgs(is_dark=True))

with open(r'C:\Users\SAZUZ\Nystic-Shadow\light.svg', 'w', encoding='utf-8') as f:
    f.write(build_perfect_svgs(is_dark=False))

print("Perfect dithered avatar SVGs from user screenshot created successfully!")
