...
def process_avatar_to_dots():
    img = Image.open(r'C:\Users\SAZUZ\Nystic-Shadow\avatar.png').convert('L')
    
    # Target size for banner layout
    target_w, target_h = 180, 180
    img = img.resize((target_w, target_h), Image.Resampling.LANCZOS)
    
    dots = []
    
    # Offset coordinates in SVG (center avatar around x=180, y=190)
    start_x = 90
    start_y = 90
    ...
    for y in range(target_h):
        for x in range(target_w):
            ...
            # Brightness threshold / sampling for dither
            # Dark pixels (val < 180) represent facial/avatar features
            if val < 210:
                prob = (255 - val) / 255.0
                if random.random() < prob * 0.75:
                    px = start_x + (x * 1.5)
                    py = start_y + (y * 1.5)
                    dots.append((round(px, 1), round(py, 1)))
                    
    print(f"Extracted {len(dots)} dithered dots from avatar.png!")
    return dots

def build_animated_svg(is_dark=True):
    dots = process_avatar_to_dots()
    ...
    # Partition dots into 30 SMIL animation groups
    num_groups = 30
    dots_per_group = max(1, len(dots) // num_groups)
    ...
    for i in range(num_groups):
        ...
        dx = round(random.uniform(-2, 2), 1)
        dy = round(random.uniform(-2, 2), 1)
      ...
      <animate attributeName="opacity" values="0.35;1;0.5;0.95;0.35" dur="{dur}s" begin="{begin_delay}s" repeatCount="indefinite"/>
    ...
    travellers_xml = []
    for _ in range(120):
        sx = random.uniform(100, 260)
        sy = random.uniform(100, 260)
        tx = random.uniform(370, 430)
        ty = random.uniform(110, 310)
  ...
  <!-- LEFT SIDE: Animated Dithered Avatar from user avatar.png -->
  <g transform="translate(10, 20)">
{portrait_svg_block}
{travellers_svg_block}

    <!-- Name & Subtitle overlay under portrait -->
    <text x="225" y="390" text-anchor="middle" fill="url(#accent)" font-size="32" font-weight="800" font-family="'Segoe UI', sans-serif" letter-spacing="0.5">Safir Akhtar</text>
    <text x="225" y="415" text-anchor="middle" fill="{text_sub}" font-size="14" font-weight="500" font-family="'Segoe UI', sans-serif">17 y/o Full-Stack Dev &amp; Creative Technologist</text>
    <text x="225" y="440" text-anchor="middle" fill="{text_accent}" font-size="15" font-weight="700" font-family="monospace">&lt;/&gt; Developer &amp; AI Automation Specialist</text>
    ...
    <g transform="translate(45, 460)">
      <rect width="470" height="75" rx="14" fill="{header_bg}" stroke="{border_color}" stroke-width="1.5"/>
...
print("Avatar-dithered dark.svg and light.svg generated successfully!")
