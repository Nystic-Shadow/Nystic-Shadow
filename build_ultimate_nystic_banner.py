...
def extract_character_dots():
    avatar = Image.open(r'C:\Users\SAZUZ\Pictures\ChatGPT Image May 10, 2026, 07_50_53 PM.png').convert('RGB')
    
    # Target size in SVG canvas
    grid_w, grid_h = 75, 75
    avatar_small = avatar.resize((grid_w, grid_h), Image.Resampling.LANCZOS)
    ...
    for gy in range(grid_h):
        for gx in range(grid_w):
            r, g, b = avatar_small.getpixel((gx, gy))
            # Calculate brightness
            brightness = (0.299 * r + 0.587 * g + 0.114 * b) / 255.0
            
            # Sample non-white / subject pixels
            if brightness < 0.92:
                prob = (1.0 - brightness)
                if random.random() < prob * 0.8:
                    px = start_x + (gx * spacing) + random.uniform(-0.2, 0.2)
                    py = start_y + (gy * spacing) + random.uniform(-0.2, 0.2)
                    dots.append((round(px, 1), round(py, 1)))

    print(f"Extracted {len(dots)} character dots from ChatGPT image!")
    return dots

def build_ultimate_svg(is_dark=True):
    dots = extract_character_dots()
    ...
    # Partition dots into 30 SMIL animation bands
    num_groups = 30
    ...
    for i in range(num_groups):
        ...
        dx = round(random.uniform(-1.2, 1.2), 1)
        dy = round(random.uniform(-1.2, 1.2), 1)
      ...
      <animate attributeName="opacity" values="0.35;1;0.5;0.95;0.35" dur="{dur}s" begin="{delay}s" repeatCount="indefinite"/>
      <animateTransform attributeName="transform" type="translate" values="0 0; {dx} {dy}; 0 0" dur="{dur*1.5:.2f}s" begin="{delay}s" repeatCount="indefinite"/>
      <path d="{path_d}"/>
    </g>'''
    ...
    travellers_xml = []
    for _ in range(120):
        sx = random.uniform(50, 290)
        sy = random.uniform(80, 310)
        tx = random.uniform(340, 400)
  ...
  <!-- LEFT COLUMN: Character Dot Matrix & Main Branding -->
...
with open(r'C:\Users\SAZUZ\Nystic-Shadow\dark.svg', 'w', encoding='utf-8') as f:
    f.write(build_ultimate_svg(is_dark=True))

with open(r'C:\Users\SAZUZ\Nystic-Shadow\light.svg', 'w', encoding='utf-8') as f:
    f.write(build_ultimate_svg(is_dark=False))

print("Ultimate Nystic Shadow character SVGs generated successfully!")
